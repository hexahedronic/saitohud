...
--
-- $Id: overlays.lua GIT 2016-02-01 21:54:24Z hexahedronic $
...
local function Draw()
	...
	for id, data in pairs(SaitoHUD.OverlayTypes) do
		...
		if data.Filter and data.HUDPaint then
			saitohud_errorhandle(data.HUDPaint)
		end

		-- Draw each entity
		for _, ent in pairs(data.Matches) do
			if IsValid(ent) then
				saitohud_errorhandle(data.DrawEnt, ent)
			end
		end
	end
end
...
local function Rehook()
	...
	if hasActive then
		...
		saitohud_errorhandle(hook.Remove, "HUDPaint", "SaitoHUD.Overlays")
	end
end
