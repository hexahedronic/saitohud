...
--
-- $Id: panel.lua GIT 2016-05-01 hexahedronic $
...
local function header(panel)
	if not panel or not IsValid(panel) then return end
	if panel.ClearControls then panel:ClearControls() end
	if panel.AddHeader then panel:AddHeader() end
end
...
function SaitoHUD.UpdatePanels()
	for k, v in pairs(panels) do
		assert(v[2])(controlpanel.Get("SaitoHUD" .. k))
	end
end
